...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="row g-3">
            <div class="col-12">
                <div class="card p-5">
                    <form action="{{ route('admin.investment.locked-in.update', $package->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group" id="name">
                                    <label class="input-label" for="exampleFormControlInput1">{{translate('messages.name')}}</label>
                                    <input value="{{ $package->name }}" type="text" name="name" class="form-control" placeholder="{{translate('messages.name')}}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group" id="amount">
                                    <label class="input-label" for="exampleFormControlInput1">{{translate('messages.amount')}}</label>
                                    <input value="{{ $package->amount }}" type="number" name="amount" class="form-control" placeholder="{{translate('messages.amount')}}" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group" id="status">
                                    ...
                                    <select name="status" class="form-control" required>
                                        <option {{ $package->status ? 'selected' : '' }} value="1">{{translate('messages.active')}}</option>
                                        <option {{ !$package->status ? 'selected' : '' }} value="0">{{translate('messages.inactive')}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
